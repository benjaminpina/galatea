...
class StageListScreen extends ConsumerWidget {
  const StageListScreen({super.key, this.embedded = false});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final body = stages.when(
      data: (list) {
        if (list.isEmpty) {
          return const Center(
            child: Text('No stages defined. Tap + to add one.'),
          );
        }
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    labelText: 'Cycles formula',
                  ),
                ),
                const SizedBox(height: 12),
                TextField(
                  controller: cond1Ctrl,
                  decoration: const InputDecoration(
                    labelText: 'Condition 1 formula',
                  ),
                ),
                ...
                DropdownButtonFormField<String>(
                  ...
                  decoration: const InputDecoration(
                    labelText: 'Cycles ↔ Requirements logic',
                  ),
                  items: const [
                  ...
                  onChanged: (v) =>
                      setState(() => logicCyclesReqs = v ?? 'AND'),
                ),
              ],
            ),
          ),
